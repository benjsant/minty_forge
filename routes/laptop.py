...
def status():
    ...
    tlp_conf_exists = Path("/etc/tlp.d/01-mintyforge.conf").exists()
...
def apply_selection():
    """Applique la selection laptop.
    Body: {
        "tlp": true/false,
        "monitoring": ["lm-sensors", "acpi", ...],
        "dock": ["HandleLidSwitchExternalPower", ...]
    }"""
    ...
    if not want_tlp and not want_monitoring and not want_dock:
        return jsonify(success=False, error="Rien a appliquer"), 400

    from scripts.laptop_setup import install_tlp, install_monitoring_selective, configure_dock_selective
...
def thermal_toggle():
    """Active ou desactive un service. Body: {"service": "...", "enable": true/false}"""
