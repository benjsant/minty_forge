...
# -*- coding: utf-8 -*-
"""
MintyForge - Profile Installer
--------------------------------
Installs all packages from a given profile (APT + Flatpak + External + Remove).
Supports deduplication when installing multiple profiles in a session.
Can be called directly or imported by web_app.
"""
...
from utils import (
    check_package_installed, apt_install, apt_remove, apt_update,
...
def install_profile(
    ...
    Args:
        slug: Profile slug name.
        seen_apt: Set of APT package names already processed this session.
        seen_flatpak: Set of Flatpak app IDs already processed this session.
        seen_external: Set of external package names already processed this session.
        dry_run: If True, only list what would be installed without executing.
    ...
    # --- APT packages ---
    if profile.apt:
        info(f"APT: {len(profile.apt)} package(s)")
...
def _dry_run_profile(profile, seen_apt, seen_flatpak, seen_external):
    ...
    for pkg in profile.apt:
        if pkg.name in seen_apt:
            info(f"  [SKIP-DUP] APT: {pkg.name}")
        elif check_package_installed(pkg.name):
            info(f"  [INSTALLED] APT: {pkg.name}")
        else:
            info(f"  [INSTALL]   APT: {pkg.name} - {pkg.description}")
        seen_apt.add(pkg.name)
    ...
    for pkg in profile.remove:
        if check_package_installed(pkg.name):
            info(f"  [REMOVE]    APT: {pkg.name} - {pkg.description}")
        else:
            info(f"  [ABSENT]    APT: {pkg.name}")
