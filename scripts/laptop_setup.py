...
from utils import (
    check_package_installed, apt_install, apt_remove,
    run_sudo_command, run_command,
    info, success, warn, error,
    get_state_manager, ACTION_APT_INSTALL, ACTION_APT_REMOVE
)
...
def install_tlp():
    ...
    for pkg in cfg.get("remove_conflicts", []):
        if check_package_installed(pkg):
            ...
            result = apt_remove([pkg])
            sm.record(
                action=ACTION_APT_REMOVE, target=pkg, success=result.success,
                rollback_cmd=["sudo", "apt", "install", "-y", pkg],
            )
    ...
    for pkg in cfg["packages"]:
        ...
        result = apt_install([pkg])
        sm.record(
            action=ACTION_APT_INSTALL, target=pkg, success=result.success,
            rollback_cmd=["sudo", "apt", "remove", "-y", pkg],
        )
        if result.success:
    ...
    if tlp_conf:
        ...
        lines = [
            "# Configuration TLP generee par MintyForge",
            "# Fichier : /etc/tlp.d/01-mintyforge.conf",
            "",
        ]
        ...
        # Ecrire dans /etc/tlp.d/ pour ne pas ecraser /etc/tlp.conf
        run_sudo_command(["mkdir", "-p", "/etc/tlp.d"])
        tmp = "/tmp/mintyforge_tlp.conf"
        Path(tmp).write_text(content)
        result = run_sudo_command(["cp", tmp, "/etc/tlp.d/01-mintyforge.conf"])
        if result.success:
            success("Configuration TLP ecrite dans /etc/tlp.d/01-mintyforge.conf")
        else:
...
def install_monitoring():
    ...
    for pkg_info in cfg["packages"]:
        ...
        sm.record(
            action=ACTION_APT_INSTALL, target=name, success=result.success,
            rollback_cmd=["sudo", "apt", "remove", "-y", name],
        )
        if result.success:
    ...
    # Lancer sensors-detect si lm-sensors est installe
    if check_package_installed("lm-sensors"):
...
def install_monitoring_selective(package_names):
    ...
    # Construire un dict nom -> description
    ...
    if "lm-sensors" in package_names and check_package_installed("lm-sensors"):
        info("Detection automatique des capteurs (sensors-detect)...")
        run_sudo_command(["sensors-detect", "--auto"])
...
def configure_dock():
    """Configure logind.conf pour un usage dock (capot ferme = pas de suspend)."""
...
def configure_dock_selective(setting_keys):
    ...
    if applied:
        ...
        if result.success:
            ...
            warn("logind.conf modifie mais restart logind echoue. Un redemarrage peut etre requis.")
...
def run_checks(live_usb_only=False):
    """Execute les diagnostics materiels et retourne les resultats."""
...
# ─── Main (si lance directement) ───────────────────────────────────────

def main():
    """Point d'entree si lance comme script standalone."""
    import sys as _sys
    if len(_sys.argv) < 2:
